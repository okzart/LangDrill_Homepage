...
| `POST /passages/generate` `{ expressions: [{ expression, meaning?, example? }], level, voice }` (called by the page's own `fetch`; JSON `{ error }` on failure). The page builds the list from picked drill items (editable) plus typed rows, max 15 | login | `POST /api/passages` (Content Sharing → llm-service + tts-service). The page plays the returned MP3 from memory and highlights each word at its `startTime`; clicking a word or expression seeks there |
